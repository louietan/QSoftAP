 ...
 */
class HostedWlan : public QObject, public std::enable_shared_from_this<HostedWlan>
{
    Q_OBJECT
  ...
  public:
     ...
     * 设置属性
     */
    WLAN_HOSTED_NETWORK_REASON setProperties(
    ...
    //获取密钥
    std::string secondaryKey() const {
        return this->secondary_key_;
    }

    std::string adapterID() const {
        return this->adapter_id_;
    }

    WLAN_HOSTED_NETWORK_STATE state() const {
        return this->state_;
    }
  ...
  signals:
    //有主机加入
    void peerJoined(std::shared_ptr<HostedWlan> sender, std::shared_ptr<WlanHost> peer);
    //有主机断开
