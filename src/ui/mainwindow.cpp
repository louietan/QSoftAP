...
void MainWindow::connectViewSignals() {
  ...
  connect(this->ui_.actionAbout, &QAction::triggered, [this](bool) {
    const auto url = "https://github.com/gemst/QSoftAP";
    QMessageBox::about(
        this, "About",
...
void MainWindow::keepViewSync() {
  ...
  connect(IcsService::instance(), &IcsService::reloadFinished,
          [this](IcsService &sender) {
  ...
  connect(&this->host_table_model_, &QAbstractTableModel::dataChanged,
          [this]() { this->ui_.tblvwHosts->viewport()->update(); });
