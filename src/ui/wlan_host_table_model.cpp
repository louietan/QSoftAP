...
QVariant WlanHostTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.row() >= this->peers.size() || index.row() < 0)
        return QVariant();
...
QVariant WlanHostTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
bool WlanHostTableModel::insertRows(int row, int count, const QModelIndex &parent)
{
...
bool WlanHostTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
...
bool WlanHostTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (index.isValid() && role == Qt::EditRole) {
        ...
        if (index.column() == 0)
        {
            ...
            std::get<1>(item) = NetUtils::readableMacAddress(PUCHAR(std::get<0>(item).data()));
        }
...
void WlanHostTableModel::onPeerJoined(std::shared_ptr<HostedWlan>, std::shared_ptr<WlanHost> peer)
{
    auto row = this->peers.size();
    this->insertRows(row, 1);
    auto idx = this->index(row, 0);
    this->setData(idx, QByteArray(reinterpret_cast<char *>(peer->peerState().PeerMacAddress)));
    ...
    if (ip.S_un.S_addr == 0)
    {
        ...
        auto update_ip = [this](QByteArray mac) {
            for (auto i = 0; i < 10; ++i)
            {
                in_addr ipv4     = {};
        ...
        std::async(update_ip, reinterpret_cast<char *>(peer->peerState().PeerMacAddress));
    }
    else
        this->setData(idx, QString::fromLocal8Bit(inet_ntoa(ip)));
}

void WlanHostTableModel::onPeerLeaved(std::shared_ptr<HostedWlan>, std::string mac)
{
