...
std::string NetUtils::readableMacAddress(unsigned char bytes[], size_t len,
                                         char delim) {
  char buffer[18] = {};
  char *cur = buffer;
...
DWORD NetUtils::findIpByMac(unsigned char mac[6]) {
  ...
  if (GetIpNetTable(arp_table.get(), &size, FALSE) ==
  ...
  for (auto i = 0; i < arp_table->dwNumEntries; ++i) {
    auto temp = readableMacAddress(arp_table->table[i].bPhysAddr,
                                   arp_table->table[i].dwPhysAddrLen);
