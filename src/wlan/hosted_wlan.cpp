...
void HostedWlan::wlanCallback(PWLAN_NOTIFICATION_DATA data, PVOID ctx)
{
    ...
    case wlan_hosted_network_state_change:
    {
        ...
        if (self->state_ != wlan_hosted_network_active)
            self->peers.clear();
    ...
    {
        auto info = static_cast<PWLAN_HOSTED_NETWORK_DATA_PEER_STATE_CHANGE>(data->pData);
        ...
        {
            auto mac = NetUtils::readableMacAddress(info->NewState.PeerMacAddress);
            auto itr = self->peers.find(mac);
            if (itr != self->peers.end())
                self->peers.erase(itr);
...
WLAN_HOSTED_NETWORK_REASON HostedWlan::setProperties(
    ...
    DWORD dwMaxNumberOfPeers)
{
    ...
    ret = WlanHostedNetworkSetSecondaryKey(
              ...
              PUCHAR(key.data()),
              TRUE,//这里表示传入的密钥是明文密码。密钥有两种形式：8到63个字节 或 64个十六进制数字（即32字节）
              TRUE,
              &reason,
              nullptr);
...
{
    if (this->state() != wlan_hosted_network_active)
        return this->turnOn();
...
DWORD HostedWlan::updateProperties()
{
    ...
    BOOL bPersistent;
    //查密钥
    auto ret = WlanHostedNetworkQuerySecondaryKey(
                   this->client_handle_,
                   &key_len,
                   &pkey,
                   &bIsPassPhrase,
...
DWORD HostedWlan::updateStatus()
{
    ...
    auto ret = WlanHostedNetworkQueryStatus(
                   this->client_handle_,
                   &status,
                   nullptr);
    ...
    if (ret == ERROR_SUCCESS)
    {
        ...
        sprintf(adapter_id.data(), "{%08lX-%04hX-%04hX-%02hhX%02hhX-%02hhX%02hhX%02hhX%02hhX%02hhX%02hhX}",
                status->IPDeviceID.Data1, status->IPDeviceID.Data2, status->IPDeviceID.Data3,
                status->IPDeviceID.Data4[0], status->IPDeviceID.Data4[1], status->IPDeviceID.Data4[2], status->IPDeviceID.Data4[3],
                status->IPDeviceID.Data4[4], status->IPDeviceID.Data4[5], status->IPDeviceID.Data4[6], status->IPDeviceID.Data4[7]);
        ...
        memcpy(this->mac_address_, status->wlanHostedNetworkBSSID, sizeof(this->mac_address_));
